<?php
/**
 * The file that defines select field render behavior.
 *
...
/**
 * Renders select field content
 *
 * @since 1.2.0
 */
class ACFP_Render_Select extends ACFP_Render {
		...
		if ( is_array( $this->field_value ) ) {
			if ( array_key_exists( 'value', $this->field_value ) ) {
				...
				if ( 'ordered_list' === $this->format && 'values' === $this->display ) {
					...
					// ordered_list_values, ordered_list_labels, unordered_list_values, unordered_list_labels.
			}
		} else {
			// single selection with ether value or label enabled.
